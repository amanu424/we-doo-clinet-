...
const TaskCard = ({ data, onDataUpdate, allTasks }) => {
  ...
  return (
    <div
      className={`task relative grid bg-white rounded-lg shadow-md px-5 py-3 border ${
        task?.status == "completed"
          ? "border-green-200 bg-gray-100 shadow-0"
          : "border-blue-200"
      }`}
    >
      <div className="flex justify-between items-center border-b py-3 border-b-gray-300">
        <div>
          ...
          <p className="text-xs italic text-gray-700 mt-1">
            {" "}
            {task.status === "taken" && task?.user?.name
              ? `${task?.user.name} is working on this. `
              : task.status === "completed" && task?.user?.name
              ? `${task?.user.name} completed this.`
              : "free task"}{" "}
          </p>
        </div>
          ...
          <div className="group relative">
            {task.status === "free" ? (
              <Tooltip title={"Click to take the task"} arrow>
                <Button
                  variant="text"
                  onClick={() => handleStatusChange("taken", "task taken")}
                >
              ...
              task.user._id === user._id ? (
                <ButtonGroup>
                  ...
                  <Tooltip title={"Click to mark as complete"} arrow>
                    <Button
                      ...
                      sx={{ color: "teal", transition: "transform .3s ease" }}
                      variant="text"
                ...
                <p className="text-xs bg-blue-200 font-bold px-2 py-1 rounded">
                  {" "}
                  taken{" "}
                </p>
              )
      ...
      >
        <Alert severity={`${msgType}`} onClose={handleCloseSnackbar}>
          {" "}
          {message}{" "}
        </Alert>
      </Snackbar>
